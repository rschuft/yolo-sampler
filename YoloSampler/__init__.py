...
# Overlay last recorded bounding boxes on the current frame
...
def overlay_bounding_boxes(frame, prev_boxes, track_history, frame_label = 'Object Tracking'):
    for (x1, y1, x2, y2, label, confidence, track_id) in prev_boxes:
...
def track_video_stream(cap, tracking_model, detection_interval = 0.25, max_track_length = 30, frame_label = 'Object Tracking'):
    ...
    while cap.isOpened():
        ...
        # Overlay previous bounding boxes on the current frame
        overlay_bounding_boxes(frame, prev_boxes, track_history, frame_label)
...
def track(tracking_model_name = 'yolov8x.pt', device_index = 0, width = 3840, height = 2160, detection_interval = 0.25, max_track_length = 30, frame_label = 'Object Tracking'):
    tracking_model = YOLO(tracking_model_name)
    cap = open_webcam(device_index, width, height)
    track_video_stream(cap, tracking_model, detection_interval, max_track_length, frame_label)
